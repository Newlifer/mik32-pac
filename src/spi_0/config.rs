...
impl W {
    #[doc = "Bit 0 - (MSTREN) Выбор режима"]
    #[inline(always)]
    pub fn mode_sel(&mut self) -> ModeSelW<'_, ConfigSpec> {
        ModeSelW::new(self, 0)
    }
    #[doc = "Bit 1 - (CPOL)Выбор полярности тактового сигнала вне слова"]
    #[inline(always)]
    pub fn clk_pol(&mut self) -> ClkPolW<'_, ConfigSpec> {
        ClkPolW::new(self, 1)
    }
    #[doc = "Bit 2 - (CPHA)Выбор фазы тактового сигнала"]
    #[inline(always)]
    pub fn clk_ph(&mut self) -> ClkPhW<'_, ConfigSpec> {
        ClkPhW::new(self, 2)
    }
    #[doc = "Bits 3:5 - Управляет скоростью передачи данных, задает коэффициент деления частоты spi_ref_clk"]
    #[inline(always)]
    pub fn baud_rate_div(&mut self) -> BaudRateDivW<'_, ConfigSpec> {
        BaudRateDivW::new(self, 3)
    }
    #[doc = "Bit 8 - Выбор опорной тактовой частоты"]
    #[inline(always)]
    pub fn ref_clk(&mut self) -> RefClkW<'_, ConfigSpec> {
        RefClkW::new(self, 8)
    }
    #[doc = "Bits 10:13 - Выбор ведомых устройств"]
    #[inline(always)]
    pub fn cs(&mut self) -> CsW<'_, ConfigSpec> {
        CsW::new(self, 10)
    }
    #[doc = "Bit 14 - Выбор режима управления сигналом выбора ведомого"]
    #[inline(always)]
    pub fn manual_cs(&mut self) -> ManualCsW<'_, ConfigSpec> {
