...
impl W {
    #[doc = "Bit 0 - (ENABLE) Включение/выключение модуля SPI"]
    #[inline(always)]
    pub fn spi_en(&mut self) -> SpiEnW<'_, EnableSpec> {
        SpiEnW::new(self, 0)
    }
    #[doc = "Bit 2 - Запись 1 при SPI_EN = 0 очищает буфер TX_FIFO."]
    #[inline(always)]
    pub fn clear_tx_fifo(&mut self) -> ClearTxFifoW<'_, EnableSpec> {
        ClearTxFifoW::new(self, 2)
    }
    #[doc = "Bit 3 - Запись 1 при SPI_EN = 0 очищает буфер RX_FIFO."]
    #[inline(always)]
    pub fn clear_px_fifo(&mut self) -> ClearPxFifoW<'_, EnableSpec> {
