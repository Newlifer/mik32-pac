...
impl W {
    #[doc = "Bit 0 - Управление питанием ЦАП"]
    #[inline(always)]
    pub fn en(&mut self) -> EnW<'_, Dac0CfgSpec> {
        EnW::new(self, 0)
    }
    #[doc = "Bit 1 - Управление сбросом ЦАП, активный уровень «0»"]
    #[inline(always)]
    pub fn rn(&mut self) -> RnW<'_, Dac0CfgSpec> {
        RnW::new(self, 1)
    }
    #[doc = "Bits 2:9 - Значение делителя тактового сигнала. Частота определяется как FЦАП=FIN/(DIV+1)"]
    #[inline(always)]
    pub fn div(&mut self) -> DivW<'_, Dac0CfgSpec> {
        DivW::new(self, 2)
    }
    #[doc = "Bit 10 - Выбор источника опорного напряжения"]
    #[inline(always)]
    pub fn exten(&mut self) -> ExtenW<'_, Dac0CfgSpec> {
        ExtenW::new(self, 10)
    }
    #[doc = "Bit 11 - Выбор источника внешнего опорного напряжения"]
    #[inline(always)]
    pub fn extpad(&mut self) -> ExtpadW<'_, Dac0CfgSpec> {
        ExtpadW::new(self, 11)
    }
}
