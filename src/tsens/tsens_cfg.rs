...
impl W {
    #[doc = "Bit 0 - Управление питанием сенсора"]
    #[inline(always)]
    pub fn npd(&mut self) -> NpdW<'_, TsensCfgSpec> {
        NpdW::new(self, 0)
    }
    #[doc = "Bit 1 - Управление тактированием сенсора"]
    #[inline(always)]
    pub fn npd_clk(&mut self) -> NpdClkW<'_, TsensCfgSpec> {
        NpdClkW::new(self, 1)
    }
    #[doc = "Bit 2 - Управление сбросом сенсора, активный уровень «0»"]
    #[inline(always)]
    pub fn nrst(&mut self) -> NrstW<'_, TsensCfgSpec> {
        NrstW::new(self, 2)
    }
    #[doc = "Bits 3:5 - Выбор источника тактирования сенсора (Fin)"]
    #[inline(always)]
    pub fn clk_mux(&mut self) -> ClkMuxW<'_, TsensCfgSpec> {
        ClkMuxW::new(self, 3)
    }
    #[doc = "Bits 6:15 - Значение делителя тактового сигнала. Частота сенсора определяется как Tsens = Fin/(2*(Div + 1))"]
    #[inline(always)]
    pub fn div(&mut self) -> DivW<'_, TsensCfgSpec> {
        DivW::new(self, 6)
    }
}
